...
<body class="bg-slate-50 font-sans antialiased text-gray-900 min-h-screen">
    ...
    <div class="bg-white shadow-sm border-b border-gray-100 py-4 px-8 flex items-center justify-between sticky top-0 z-50">
        <div class="flex items-center gap-4">
            <h1 class="text-xl font-bold text-gray-800">Pembayaran Perpanjangan</h1>
        </div>
        <div>
            <span class="bg-orange-100 text-orange-700 text-xs font-bold px-3 py-1.5 rounded-lg uppercase tracking-wide">Bayar & Verifikasi</span>
        </div>
    </div>
    ...
    <main class="max-w-3xl mx-auto px-6 py-12">
        <div class="bg-white rounded-[2rem] p-10 border border-gray-100 shadow-sm flex flex-col items-center text-center">
            ...
            <!-- Total Amount -->
            <div class="mb-8">
                <p class="text-sm text-gray-400 font-bold uppercase tracking-wider mb-1">Total Tagihan ({{ $days }} Hari)</p>
                <p class="text-blue-600 font-extrabold text-4xl">Rp {{ number_format($cost ?? 0, 0, ',', '.') }}</p>
            </div>
            ...
            <div class="w-full max-w-md flex flex-col gap-4">
                <form action="{{ route('rentals.pay_extend', $rental->id ?? 999) }}" method="POST" class="w-full">
                    @csrf
