...
<main class="max-w-5xl mx-auto px-8 py-8 w-full">
    ...
    @if($rentals->count() > 0)
        ...
        <div class="bg-white rounded-[2.5rem] shadow-xl shadow-blue-900/5 border border-gray-100 overflow-hidden mb-8">
            
            <!-- Top Status Bar -->
            <div class="px-8 py-4 flex items-center justify-between text-white {{ $rental->status == 'pending' ? 'bg-orange-500' : ($rental->status == 'rejected' ? 'bg-red-500' : 'bg-blue-600') }}" id="statusBar-{{ $rental->id }}">
            ...
            <div class="p-8 lg:p-12 flex flex-col lg:flex-row gap-12">
                ...
                <div class="flex-1 lg:border-r border-gray-100 lg:pr-12">
                    ...
                    <div>
                        <p class="text-xs text-gray-400 font-bold uppercase tracking-widest mb-3 flex items-center gap-2" id="timerTitle-{{ $rental->id }}">
                        ...
                        <div class="flex items-baseline gap-2">
                            <span class="text-5xl font-black text-gray-900 tabular-nums tracking-tight" id="countdownTimer-{{ $rental->id }}">Menghitung...</span>
                        </div>
                ...
                @if($rental->status == 'active')
                <div class="lg:w-[320px] flex-shrink-0 flex flex-col justify-center hidden" id="penaltySection-{{ $rental->id }}">
                    <div class="bg-red-50 rounded-[2rem] p-8 border border-red-100 text-center relative overflow-hidden shadow-inner shadow-red-900/5">
                        <div class="relative z-10">
                            ...
                            <p class="text-3xl font-black text-red-600 mb-6 tabular-nums tracking-tight" id="overdueTimer-{{ $rental->id }}">00:00:00</p>
            ...
            <div class="p-8 lg:px-12 lg:py-8 bg-slate-50 border-t border-gray-100 flex flex-col sm:flex-row gap-4">
                @if($rental->status == 'active')
                <button id="extendOrPenaltyBtn-{{ $rental->id }}" onclick="window.location.href = '/rentals/{{ $rental->id }}/extend'" class="flex-1 bg-white hover:bg-slate-100 text-gray-700 font-bold py-4 px-6 rounded-2xl border border-gray-200 shadow-sm transition-all duration-300 text-base flex items-center justify-center gap-2 group">
                    Ajukan Perpanjangan
                </button>
                ...
                @endif
                <button onclick="window.location.href = '/rentals/{{ $rental->id }}/invoice'" class="flex-1 bg-gray-800 hover:bg-gray-900 text-black font-extrabold py-4 px-6 rounded-2xl shadow-xl shadow-gray-800/20 transition-all duration-300 hover:-translate-y-1 text-base flex items-center justify-center gap-2 group">
        ...
        <script>
            (function() {
                ...
                const timerEl = document.getElementById(`countdownTimer-${rentalId}`);
                const overdueTimerEl = document.getElementById(`overdueTimer-${rentalId}`);
                const penaltySection = document.getElementById(`penaltySection-${rentalId}`);
                const timerTitle = document.getElementById(`timerTitle-${rentalId}`);
                const statusBar = document.getElementById(`statusBar-${rentalId}`);
                const extendOrPenaltyBtn = document.getElementById(`extendOrPenaltyBtn-${rentalId}`);
                ...
                function updateTimer() {
                    ...
                    if (difference > 0) {
                        ...
                        if (timerEl) timerEl.innerText = formatTime(difference);
                        ...
                        if (statusBar) {
                            ...
                            let statusTextEl = statusBar.querySelector('span');
                            if (statusTextEl) statusTextEl.innerText = 'STATUS: TERLAMBAT PENGEMBALIAN';
                        }

                        // Tampilkan box denda
                        if (penaltySection) penaltySection.classList.remove('hidden');
                        if (overdueTimerEl) overdueTimerEl.innerText = formatTime(overdueMs);
                        ...
                        if (extendOrPenaltyBtn) {
                            if (penaltyStatus === 'pending_verification') {
                                ...
                                if (extendOrPenaltyBtn.innerText !== "Bayar Denda (Rp 100.000)") {
                                    ...
                                    extendOrPenaltyBtn.onclick = () => {
                                        const modal = document.getElementById(`penaltyModal-${rentalId}`);
                ...
                // Pengecekan otomatis ke backend jika status berubah
        ...
        <div id="penaltyModal-{{ $rental->id }}" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div class="bg-white rounded-[2rem] w-full max-w-md p-8 shadow-2xl relative">
                <button onclick="document.getElementById('penaltyModal-{{ $rental->id }}').classList.add('hidden')" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600">
                ...
                <div class="flex flex-col gap-3">
                    <button onclick="submitPenalty({{ $rental->id }})" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-xl transition-colors">
                        Saya Sudah Transfer
                    </button>
        ...
        <script>
            function submitPenalty(rentalId) {
